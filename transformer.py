...
class transformer:
    ...
    def applyTransforms(self, frame):
        ...
        min_horz, max_horz, min_vert, max_vert = self.__find_framing_lines(orth_lines, frame)

        # Calculate the corner points
        tl = self.__line_intersections(min_horz[0], min_horz[1], min_vert[0], min_vert[1]) # Upper left
        tr = self.__line_intersections(min_horz[0], min_horz[1], max_vert[0], max_vert[1]) # Upper right
        bl = self.__line_intersections(max_horz[0], max_horz[1], min_vert[0], min_vert[1]) # Lower left
        br = self.__line_intersections(max_horz[0], max_horz[1], max_vert[0], max_vert[1]) # Lower right
    ...
    def __find_lines(self, frame):
        ...
        def dfunc(frame, lines, drawline):
            ...
            return frame
        self.debugger.addFrame('Lines', frame.copy(), dfunc, lines[0], self.__draw_line)
    ...
    def __find_ortho_lines(self, lines, frame):
        ...
        for rho,theta in lines:
            ...
            if ((dtheta%180 < self.angle_threshold) or (abs(dtheta%180-180) < self.angle_threshold)):
                vert_lines.append([rho,theta])
            elif (((dtheta-90)%180 < self.angle_threshold) or (abs((dtheta-90)%180-180) < self.angle_threshold)):
        ...
        def dfunc(frame, vert_line, horz_line, drawline):
            ...
            return frame
        self.debugger.addFrame('Ortho Lines', frame.copy(), dfunc, vert_lines, horz_lines, self.__draw_line)
    ...
    def __find_framing_lines(self, orth_lines, frame):
        ...
        if (min_horz is None or max_horz is None or min_vert is None or max_vert is None):
        ...
        def dfunc(frame, min_vert, min_horz, drawline):
            ...
            return frame
        self.debugger.addFrame('Framing', frame.copy(), dfunc, min_vert, min_horz, self.__draw_line)
